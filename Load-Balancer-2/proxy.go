...
func (proxy Proxy) handler(w http.ResponseWriter, r *http.Request) {
	...
	*counterPtr++
	...
	//log.Println(*counterPtr)
	if *counterPtr >= 1000 {
		color.Yellow("To avoid port crash, Re-diecting to load balancer on port 10000")

		http.Redirect(w, r, "http://localhost:10000", http.StatusSeeOther)
